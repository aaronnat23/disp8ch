...
async function main() {
  ...
  try {
    ...
    const notebookHeading = page.getByRole("heading", { name: notebookName, exact: true });
    await notebookHeading.waitFor({ state: "visible", timeout: 30000 });
    check("notebooks tab shows seeded notebook", await notebookHeading.isVisible());
    ...
    check("notebooks tab shows source file", await page.getByText(fileName, { exact: false }).first().isVisible());
