...
async function main() {
  ...
  {
    const sendEffect = resolveNodeEffect("send-telegram", { message: "{{agent.response}}" });
  ...
  console.log("\nExpired grants cannot be approved");
  {
    const expired = createApprovalRequest({
      ...
      nodeId: "node",
      attempt: 1,
      effect: resolveNodeEffect("send-telegram", { message: "expired" }),
      inputHash: "input",
  ...
  // node TYPE drives effect classification; "send-telegram" → external_send
  // (needs approval under balanced), "send-webchat"/"memory-store" → local_write (auto).
  let sendCalls = 0;
  let failSend = false;
  registerNodeHandler({ type: "send-telegram", async execute(input) {
    sendCalls++;
  ...
  registerNodeHandler({ type: "send-webchat", async execute(input) {
    webchatCalls++;
    return { data: { ...input.data, sent: true, localWebChat: true } };
  } });
  let hardlineCalls = 0;
  ...
  const sendNode = { id: "s1", type: "send-telegram", position: { x: 200, y: 0 }, data: { label: "Reply", message: "hi" } };
  const webchatNode = { id: "wc1", type: "send-webchat", position: { x: 200, y: 0 }, data: { label: "Local WebChat Reply", message: "hi" } };
  const edge = { id: "e1", source: "t1", target: "s1" };
  const webchatEdge = { id: "e-wc", source: "t1", target: "wc1" };
  ...
  // ── 1b. Local WebChat reply runs without approval under balanced mode ──
  console.log("\nLocal WebChat reply does not require external-send approval");
  makeWorkflow("wf-webchat-local", [triggerNode, webchatNode], [webchatEdge], "balanced");
  webchatCalls = 0;
  {
    const result = await runManual("wf-webchat-local", [triggerNode, webchatNode], [webchatEdge]);
    check("webchat reply handler runs without approval", webchatCalls === 1, `webchatCalls=${webchatCalls}`);
    check("webchat reply execution completed", result.status === "completed", result.status);
  }
  ...
  const loopBodySend = { id: "lb", type: "send-telegram", position: { x: 400, y: 0 }, data: { label: "LoopSend", message: "x" } };
