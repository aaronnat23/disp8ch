  ...
  try {
    ...
    if (support) {
      ...
      const execution = draft
        ...
        : null;
      if (!draft) {
        check("support triage has draft target", false);
      }
      ...
      const sendNode = nodeByLabel(support, "Present Draft For Approval");
      check(
        "support triage keeps user-visible send as a separate approval boundary",
        sendNode?.type === "send-webchat",
        sendNode ? `${sendNode.id}:${sendNode.type}` : "missing",
      );
      /*
       * Do not execute through Present Draft For Approval in this smoke. The
       * workflow effect guard correctly pauses external_send nodes for human
       * approval, so a headless test would wait for an approval grant.
       */
    }
  } finally {
